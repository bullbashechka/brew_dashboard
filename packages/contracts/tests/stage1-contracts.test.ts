...
describe("Stage 1 shared contracts", () => {
  ...
  it("rejects unknown mutation fields and invalid writeoff quantities", () => {
    expect(priceMutationSchema.safeParse({ price: "12.50" }).success).toBe(true);
    expect(priceMutationSchema.safeParse({ price: "12.5" }).success).toBe(false);
    expect(priceMutationSchema.safeParse({ price: "12.50", extra: true }).success).toBe(false);
    expect(
  ...
  it("keeps error envelopes and reset payload strict", () => {
    ...
    expect(resetMutationSchema.safeParse({}).success).toBe(true);
    expect(resetMutationSchema.safeParse({ confirm: true }).success).toBe(false);
